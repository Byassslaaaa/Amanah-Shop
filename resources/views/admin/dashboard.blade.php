        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 mt-1">Rp{{ number_format($monthlyIncome, 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600 mt-1">Rp{{ number_format($monthlyExpense, 0, ',', '.') }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.finance.report') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Lihat Detail
                </a>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.credits.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Kelola
                </a>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="p-4 bg-orange-50 rounded-lg">
                    ...
                    <p class="text-xl font-bold text-orange-900">Rp{{ number_format($totalReceivable, 0, ',', '.') }}</p>
                </div>

                <div class="p-4 bg-green-50 rounded-lg">
                    <p class="text-sm text-green-600 mb-1">Total Terbayar</p>
                    <p class="text-xl font-bold text-green-900">Rp{{ number_format($totalCreditPaid, 0, ',', '.') }}</p>
                </div>

                @if($overdueAmount > 0)
                <div class="p-4 bg-red-50 rounded-lg">
                    <p class="text-sm text-red-600 mb-1">Tunggakan</p>
                    <p class="text-xl font-bold text-red-900">Rp{{ number_format($overdueAmount, 0, ',', '.') }}</p>
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.products.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Lihat Semua
                </a>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div class="flex items-center flex-1 min-w-0">
                        <div class="w-10 h-10 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg flex items-center justify-center mr-3 flex-shrink-0 overflow-hidden">
                            @if($product->images && count($product->images) > 0)
                                <img src="{{ $product->getImageDataUri(0) }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                            @else
                    ...
                    <span class="ml-3 px-2 py-1 text-xs font-semibold {{ $product->stock == 0 ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }} rounded-full">
                        Stok: {{ $product->stock }}
                    </span>
                </div>
                @empty
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.credit-payments.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Lihat Semua
                </a>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:border-blue-300 transition-colors">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-900 truncate">{{ $payment->manualCredit->customer_name }}</p>
                    ...
                    <div class="ml-3 text-right">
                        <p class="text-sm font-semibold text-gray-900">Rp{{ number_format($payment->amount_due - $payment->amount_paid, 0, ',', '.') }}</p>
                    </div>
                </div>
                @empty
                <div class="text-center py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.finance.transactions.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Lihat
                </a>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 border-l-4 {{ $transaction->type == 'income' ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50' }} rounded">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-900 truncate">{{ $transaction->category->name }}</p>
                        <p class="text-xs text-gray-500">{{ $transaction->transaction_date->format('d M Y') }}</p>
                    </div>
                    <p class="ml-3 text-sm font-semibold {{ $transaction->type == 'income' ? 'text-green-600' : 'text-red-600' }}">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.inventory.movements.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                    Lihat
                </a>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 border-l-4 {{ $movement->type == 'in' ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50' }} rounded">
                    <div class="flex-1 min-w-0">
                    ...
                    <p class="ml-3 text-sm font-semibold {{ $movement->type == 'in' ? 'text-green-600' : 'text-red-600' }}">
        ...
        new Chart(financeTrendCtx, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Pemasukan',
                        data: @json(array_map(function($item) { return $item['income']; }, $financeTrend)),
                        borderColor: '#10B981',
                    ...
                    {
                        label: 'Pengeluaran',
                        data: @json(array_map(function($item) { return $item['expense']; }, $financeTrend)),
                        borderColor: '#EF4444',
