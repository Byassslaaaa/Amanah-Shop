...
@section('content')
    ...
    <section class="bg-white py-8" x-data="{
        currentSlide: 0,
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($banners->count() > 0)
                <!-- Carousel Container -->
                <div class="relative rounded-3xl overflow-hidden mb-6 shadow-sm">
                    <!-- Slides -->
                    <div class="relative min-h-[420px]">
                        @foreach($banners as $index => $banner)
                            <div x-show="currentSlide === {{ $index }}"
                                 ...
                                 style="display: none;">
                                <div class="bg-gradient-to-r from-[#BCE3C9] via-[#9FD3B8] to-[#BCE3C9] p-12 min-h-[420px] relative overflow-hidden">
                                    ...
                                    <div class="relative z-10 max-w-2xl">
                                        ...
                                        @if($banner->subtitle)
                                            <p class="text-[20px] text-[#7E7E7E] mb-8 font-medium">{{ $banner->subtitle }}</p>
                                        @endif

                                        @if($banner->button_text && $banner->button_link)
                                            <a href="{{ $banner->button_link }}"
                                               class="inline-block bg-[#3BB77E] hover:bg-[#2a9d66] text-white font-bold px-8 py-4 rounded-full transition-all duration-300 shadow-lg hover:shadow-xl active:scale-95">
                                                {{ $banner->button_text }}
                                            </a>
                                        @endif
                                    </div>
                    ...
                    @if($banners->count() > 1)
                        <button @click="prevSlide(); stopAutoSlide(); startAutoSlide();"
                                class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-[#253D4E] w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 z-20">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </button>
                        <button @click="nextSlide(); stopAutoSlide(); startAutoSlide();"
                                class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-[#253D4E] w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 z-20">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-2 z-20">
                            @foreach($banners as $index => $banner)
                                <button @click="goToSlide({{ $index }})"
                                        :class="currentSlide === {{ $index }} ? 'bg-[#3BB77E] w-8' : 'bg-white/50 w-3'"
                                        class="h-3 rounded-full transition-all duration-300 hover:bg-[#3BB77E]">
                                </button>
                            @endforeach
                        </div>
                    @endif
            ...
            @else
                ...
                <div class="bg-gradient-to-r from-[#BCE3C9] via-[#9FD3B8] to-[#BCE3C9] rounded-3xl p-12 mb-6 relative overflow-hidden min-h-[420px] shadow-sm">
                    <!-- Decorative Circles -->
                    <div class="absolute inset-0 overflow-hidden pointer-events-none">
                        <div class="absolute top-20 right-40 w-32 h-32 rounded-full bg-white opacity-10 animate-pulse"></div>
                        <div class="absolute bottom-20 right-20 w-24 h-24 rounded-full bg-white opacity-10 animate-pulse" style="animation-delay: 0.5s;"></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div class="bg-[#ECFFEC] rounded-2xl p-6 hover:shadow-md transition-all relative overflow-hidden group cursor-pointer">
                    <div class="relative z-10">
                        <p class="text-[13px] text-gray-600 mb-1">Koleksi Premium</p>
                        <h3 class="text-[26px] font-bold text-[#253D4E] mb-1.5">Tekstil</h3>
                        <p class="text-[13px] text-gray-600 mb-3">Kualitas terbaik</p>
    ...
    <section class="py-12 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-8">
                <h2 class="text-[32px] font-bold text-[#253D4E]">Penawaran Hari Ini</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                @foreach ($featuredProducts->take(4) as $product)
                    <div class="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 group border border-gray-200">
                        <!-- Product Image -->
                        ...
                        <div class="p-4">
                            <!-- Product Name -->
                            <a href="{{ route('products.show', $product) }}">
                                <h3 class="font-bold text-[#253D4E] text-[15px] mb-2 line-clamp-2 group-hover:text-[#3BB77E] transition-colors leading-snug min-h-[2.8rem]">
                                    {{ $product->name }}
                                </h3>
                            </a>
                            ...
                            <p class="text-[13px] text-gray-500 mb-2">
                                Oleh <a href="{{ route('products.index') }}" class="text-[#3BB77E] hover:text-[#2a9d66] transition-colors">Amanah Shop</a>
                            </p>
                            ...
                            <div class="flex items-center gap-1 mb-3">
                                ...
                                @for ($i = 1; $i <= 5; $i++)
                                    @if ($i <= $fullStars)
                                        <svg class="w-4 h-4 text-[#FDC040] fill-current" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                        </svg>
                                    @else
                                    ...
                                    @endif
                                @endfor
                                <span class="text-[12px] text-gray-400 ml-1">({{ number_format($avgRating, 1) }})</span>
                            </div>

                            <!-- Price -->
                            <div class="flex items-center gap-2 mb-3">
                                <span class="text-[18px] font-bold text-[#3BB77E]">
                                    Rp{{ number_format($product->price, 0, ',', '.') }}
                                </span>
                                @php
                            ...
                            <div onclick="event.stopPropagation()">
                                @auth
                                    <form action="{{ route('user.cart.add') }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="w-full flex items-center justify-center gap-2 bg-[#DEF9EC] hover:bg-[#3BB77E] text-[#3BB77E] hover:text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-300 text-[13px]">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-gradient-to-br from-[#FFF8E5] to-[#FFEDCC] rounded-2xl p-8 relative overflow-hidden group hover:shadow-lg transition-all cursor-pointer">
                    <div class="relative z-10">
                        <p class="text-[13px] text-[#FF9933] font-semibold mb-1">Furniture</p>
                        <h3 class="text-[32px] font-bold text-[#253D4E] leading-tight mb-3">
                            Diskon 20%<br>
                            <span class="text-[28px]">untuk <span class="text-[#3BB77E]">Perabotan</span><br>Rumah</span>
                        </h3>
                ...
                <div class="bg-gradient-to-br from-[#FFE8E8] to-[#FFD4D4] rounded-2xl p-8 relative overflow-hidden group hover:shadow-lg transition-all cursor-pointer">
                    <div class="relative z-10">
                        <h3 class="text-[28px] font-bold text-[#253D4E] leading-tight mb-3">
                            Fashion Terbaru<br>untuk Gaya Hidup<br>Modern Anda
                        </h3>
                        <a href="{{ route('products.index') }}"
                            class="inline-flex items-center gap-2 bg-[#3BB77E] hover:bg-[#2a9d66] text-white font-bold px-6 py-2.5 rounded-md transition-all text-[13px]">
                            <span>Belanja Sekarang</span>
                ...
                <div class="bg-gradient-to-br from-[#E5F5FF] to-[#CCE9FF] rounded-2xl p-8 relative overflow-hidden group hover:shadow-lg transition-all cursor-pointer">
                    <div class="relative z-10">
                        <h3 class="text-[28px] font-bold text-[#253D4E] leading-tight mb-3">
                            Perlengkapan<br>Rumah Tangga<br>Berkualitas
                        </h3>
                        <a href="{{ route('products.index') }}"
                            class="inline-flex items-center gap-2 bg-[#3BB77E] hover:bg-[#2a9d66] text-white font-bold px-6 py-2.5 rounded-md transition-all text-[13px]">
                            <span>Belanja Sekarang</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div>
                    <div class="mb-6">
                        ...
                        <div class="w-16 h-0.5 bg-[#3BB77E]"></div>
                    </div>
                    <div class="space-y-5">
                        @foreach ($topSellingProducts as $product)
                            <div class="flex gap-4 group">
                                <a href="{{ route('products.show', $product) }}" class="flex-shrink-0">
                                    @if ($product->images && count($product->images) > 0)
                                        <img src="{{ $product->getImageDataUri(0) }}" alt="{{ $product->name }}"
                                            class="w-20 h-20 object-contain rounded-lg bg-white border border-gray-100 p-2">
                                    @else
                                        <div class="w-20 h-20 bg-gray-50 rounded-lg flex items-center justify-center border border-gray-100">
                                            <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                            </svg>
                                        </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('products.show', $product) }}">
                                        <h4 class="font-semibold text-[#253D4E] text-[15px] mb-2 line-clamp-2 group-hover:text-[#3BB77E] transition-colors leading-tight">
                                            {{ $product->name }}
                                        </h4>
                                    </a>
                                    <div class="flex items-center gap-1 mb-2">
                                        ...
                                        @for ($i = 1; $i <= 5; $i++)
                                            ...
                                            @endif
                                        @endfor
                                        <span class="text-[11px] text-gray-400 ml-1">{{ $reviewsCount }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[16px] font-bold text-[#3BB77E]">
                                            Rp{{ number_format($product->price, 0, ',', '.') }}
                                        </span>
                                        <span class="text-[13px] text-gray-400 line-through">
                                            Rp{{ number_format($product->price * 1.3, 0, ',', '.') }}
